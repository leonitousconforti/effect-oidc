...
export const ProtectedHeader = Schema.StructWithRest(
    Schema.Struct({
        ...
        /**
         * Agreement PartyUInfo for ECDH-ES (base64url).
         *
        ...
        /**
         * Agreement PartyVInfo for ECDH-ES (base64url).
         *
        ...
        /**
         * Initialization Vector for AES-GCM key wrap (base64url).
         *
         * @see https://www.rfc-editor.org/rfc/rfc7518#section-4.7.1.1
         */
        iv: Schema.String.pipe(Schema.optional),

        /**
         * Authentication Tag for AES-GCM key wrap (base64url).
         *
         * @see https://www.rfc-editor.org/rfc/rfc7518#section-4.7.1.2
         */
...
/**
 * 64-bit big-endian encoding of a bit length.
 *
 * @internal
 */
...
/**
 * Constant-time byte comparison (length is not treated as secret).
 *
 * @internal
 */
...
/**
 * RFC 7518 Section 4.6.2 Concat KDF specialised to SHA-256.
 *
 * @internal
 */
const concatKdf = Effect.fnUntraced(function* (
    ...
    for (let i = 1; i <= reps; i++) {
        ...
        const digest = new Uint8Array(
            yield* Effect.promise(() =>
                crypto.subtle.digest("SHA-256", u8(concatBytes(counter, sharedSecret, otherInfo)))
            )
        );
...
const aesKwUnwrap = (kek: CryptoKey, wrapped: Uint8Array) =>
    Effect.gen(function* () {
        const cekKey = yield* Effect.tryPromise({
            try: () =>
                crypto.subtle.unwrapKey("raw", u8(wrapped), kek, "AES-KW", { name: "HMAC", hash: "SHA-256" }, true, [
                    "sign",
                ]),
        ...
        return new Uint8Array(
            yield* Effect.tryPromise({
                try: () => crypto.subtle.exportKey("raw", cekKey),
                catch: die("KeyManagementFailed"),
            })
        );
    });
...
const keyManagementEncrypt = Effect.fnUntraced(function* (
    ...
    switch (alg) {
        ...
        case "PBES2-HS512+A256KW": {
            const hash = alg.startsWith("PBES2-HS256")
                ? "SHA-256"
...
const keyManagementDecrypt = Effect.fnUntraced(function* (
    ...
    switch (alg) {
        ...
        case "A192GCMKW":
        case "A256GCMKW": {
            if (header.iv === undefined || header.tag === undefined)
        ...
        case "PBES2-HS384+A192KW":
        case "PBES2-HS512+A256KW": {
            if (header.p2s === undefined || header.p2c === undefined)
                return yield* new JweError({ reason: "Malformed" });
            ...
            const kekBits = new Uint8Array(
                yield* Effect.tryPromise({
                    try: () =>
                        crypto.subtle.deriveBits(
                            { name: "PBKDF2", salt: u8(salt), iterations: p2c, hash },
                            key,
                            aesKwBits(alg)
                        ),
                    catch: die("KeyManagementFailed"),
...
export const encrypt = Effect.fnUntraced(function* (options: {
    ...
    const plaintextBytes =
        typeof options.plaintext === "string" ? textEncoder.encode(options.plaintext) : options.plaintext;

    const { ciphertext, tag } = yield* contentEncrypt(params, km.cek, iv, plaintextBytes, aad);

    return [protectedB64, base64Url(km.encryptedKey), base64Url(iv), base64Url(ciphertext), base64Url(tag)].join(".");
});
...
export const decrypt = Effect.fnUntraced(function* (options: {
    ...
    if (
        options.contentEncryptionAlgorithms !== undefined &&
        !options.contentEncryptionAlgorithms.includes(header.enc)
    ) {
        return yield* new JweError({ reason: "UnsupportedAlgorithm" });
    }
